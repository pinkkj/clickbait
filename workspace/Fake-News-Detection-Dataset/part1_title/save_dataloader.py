...
if __name__ == '__main__':
    ...
    print("CWD:", os.getcwd())
    print("CFG data_path:", cfg["DATASET"]["data_path"])
    print("exists?", os.path.exists(cfg["DATASET"]["data_path"]))
    print("listdir:", os.listdir(cfg["DATASET"]["data_path"]) if os.path.exists(cfg["DATASET"]["data_path"]) else None)
    ...
    for split in cfg['MODE']['save_list']:
        split_dir = os.path.join(cfg["DATASET"]["data_path"], split)
        print("split_dir:", split_dir, "exists?", os.path.isdir(split_dir))
        try:
            ...
            print(f"[ERROR] split={split}: {repr(e)}")
            traceback.print_exc()
            raise
